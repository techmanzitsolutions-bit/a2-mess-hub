...
for app in paths:
    ...
    # Kitchen routine:
    # Breakfast -> same-day 06:00 cutoff.
    # Dinner -> same-day 17:00 cutoff.
    # Lunch -> prepared with the previous evening's dinner, so previous-day 17:00 cutoff.
    ...
    label="<div class=small>${meal==='Lunch'?'Prepared previous evening · cutoff previous day 17:00':meal==='Dinner'?'Cutoff today 17:00':'Cutoff today 06:00'}</div>"
    s=s.replace("<div class=small>Daily skip cutoff 17:00</div>",label)
    s=s.replace("<div class=small>Cutoff ${mealSkipCutoffs[meal]}</div>",label)
    ...
    base_note="<div class=note style=\"margin-top:12px\">Skip Meal changes cooking quantity only. It does not reduce your monthly payment or expense share.</div>"
    kitchen_note="<div class=note style=\"margin-top:12px\"><b>Kitchen timing:</b> Dinner and the next day’s Lunch are prepared together in the evening. Dinner must be skipped before 5:00 PM that day; Lunch must be skipped before 5:00 PM on the previous day. Closed meals do not show a Skip option. Future dates remain available.</div>"
    if old_note in s:
        s=s.replace(old_note,kitchen_note,1)
    elif 'Dinner and the next day’s Lunch are prepared together' not in s and base_note in s:
        s=s.replace(base_note,kitchen_note+base_note,1)
