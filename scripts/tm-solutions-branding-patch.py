...
BRAND='''<div class="powered tm-powered" style="margin-top:18px;display:grid;justify-items:center;gap:4px;color:#7f9d91;font-size:9px;letter-spacing:.08em;text-transform:uppercase"><span>Powered by</span><img src="./tm-solutions-logo.webp" alt="TM Solutions" style="width:112px;max-width:82%;height:auto;display:block;filter:drop-shadow(0 0 9px rgba(38,160,255,.34))"></div>'''
SPLASH_BRAND='''<div class="tech tm-powered" style="position:fixed;bottom:20px;left:0;right:0;display:grid;justify-items:center;gap:4px;color:#8ca89d;font-size:9px;letter-spacing:.08em;text-transform:uppercase"><span>Powered by</span><img src="./tm-solutions-logo.webp" alt="TM Solutions" style="width:112px;max-width:48vw;height:auto;display:block;filter:drop-shadow(0 0 9px rgba(38,160,255,.34))"></div>'''
...
for page in paths:
    ...
    if page.name == 'app.html':
        ...
        s=re.sub(r'<p\b[^>]*>\s*(?:⚡\s*)?POWERED BY\s+TECHMANZ\s*</p>', BRAND, s, flags=re.I)
        # Replace any already-injected legacy powered element that survived another patch.
        ...
        s=s.replace('Powered by TECHMANZ','Powered by TM Solutions')
        s=s.replace('POWERED BY TECHMANZ','POWERED BY TM SOLUTIONS')
        if 'tm-solutions-logo.webp' not in s:
            # Fallback for an alternate splash generated by older build steps.
            s=s.replace('</body>', SPLASH_BRAND+'</body>', 1)
        if 'tm-solutions-logo.webp' not in s:
            raise SystemExit('TM Solutions splash branding was not applied')

    page.write_text(s)
    print('Applied TM Solutions powered-by branding to', page)
