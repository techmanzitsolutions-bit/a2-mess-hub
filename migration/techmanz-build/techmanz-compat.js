...
export async function createUserWithEmailAndPassword(auth,email,password){const r=await api('/compat/auth/create',{method:'POST',body:JSON.stringify({email,password,name:email.split('@')[0]})});auth.currentUser={uid:r.uid,email:r.email};return{user:auth.currentUser}}
